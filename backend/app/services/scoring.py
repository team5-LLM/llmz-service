...
# 구 5단계·hold 등 → 현행 3단계(proceed/review/low_priority)
...
def adoption_decision(opportunity_score: int, risk_score_value: float) -> dict:
    ...
    if opportunity_score >= 50:
        if opportunity_score >= 70 and risk_score_value > 60:
            message = (
                "자동화 가치는 높지만 보안 위험도가 높아 "
                "즉시 도입보다 보안 검토가 필요합니다."
            )
            required_action = (
                "Private 환경 검토, 원문 저장 금지, 권한 통제, 감사 로그 적용"
            )
        elif opportunity_score >= 70:
            message = (
                "자동화 가치는 높지만 일부 민감정보 가능성이 있어 "
                "기본 보안 조치가 필요합니다."
            )
            required_action = (
                "마스킹 정책 적용, 접근 권한 제한, 로그 보관 기간 설정"
            )
        else:
