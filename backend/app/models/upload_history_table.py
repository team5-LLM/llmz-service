...
from sqlalchemy import Index, Integer, Unicode, UnicodeText
...
class UploadHistoryRow(Base):
    ...
    validation_errors_json: Mapped[Optional[str]] = mapped_column(UnicodeText, nullable=True)
    blob_path: Mapped[Optional[str]] = mapped_column(Unicode(1024), nullable=True)
    blob_purged_at: Mapped[Optional[str]] = mapped_column(Unicode(40), nullable=True)

    status: Mapped[str] = mapped_column(Unicode(32), nullable=False, default="pending")
    status_history_json: Mapped[str] = mapped_column(UnicodeText, nullable=False, default="[]")
    error_message: Mapped[Optional[str]] = mapped_column(UnicodeText, nullable=True)

    summary_json: Mapped[Optional[str]] = mapped_column(UnicodeText, nullable=True)
    completed_at: Mapped[Optional[str]] = mapped_column(Unicode(40), nullable=True)
